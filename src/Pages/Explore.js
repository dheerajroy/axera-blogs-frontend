...
export default function Explore() {
    ...
    useEffect(() => {
        document.title = 'Axera Blogs - Explore'
    }, [])

    useEffect(() => {
        if (nextPage) {
            axios.get(`${apiUrl}/api/blogs-aggregator/`, { params: { page: pageNo, topic__topic: topic, search: q } })
                .then(response => {
    ...
    return (
        <Layout page={
            <>
                ...
                <center><Link to={`/?q=${q}&topic=${topic}`}>Check out our articles.</Link></center>
            </>
        } />
    )
}
